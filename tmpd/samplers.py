...
def get_cs_sampler(config, sde, model, sampling_shape, inverse_scaler, y, num_y, H, observation_map, adjoint_observation_map, stack_samples=False):
    ...
    if config.sampling.cs_method.lower()=='projectionkalmanfilter':
        ...
        outer_solver = DPSDDPMplus(scale, y, observation_map, sampling_shape[1:], score, num_steps=config.solver.num_outer_steps,
        ...
        scale = 1.0
        # Designed to replicate dps paper as closely as possible
        outer_solver = DPSDDPM(scale, y, observation_map, sampling_shape[1:], score, num_steps=config.solver.num_outer_steps,
                            dt=config.solver.dt, epsilon=config.solver.epsilon,
                            beta_min=config.model.beta_min,
                            beta_max=config.model.beta_max)
        sampler = get_sampler(sampling_shape, outer_solver, inverse_scaler=inverse_scaler, stack_samples=stack_samples, denoise=True)
        ...
        score = model
        scale = 1.0
        outer_solver = DPSSMLDplus(scale, y, observation_map, sampling_shape[1:], score, num_steps=config.solver.num_outer_steps,
                            dt=config.solver.dt, epsilon=config.solver.epsilon,
        ...
        score = model
        scale = 1.0
        outer_solver = DPSSMLD(scale, y, observation_map, sampling_shape[1:], score, num_steps=config.solver.num_outer_steps,
                            dt=config.solver.dt, epsilon=config.solver.epsilon,
                            sigma_min=config.model.sigma_min,
                            sigma_max=config.model.sigma_max)
        sampler = get_sampler(sampling_shape, outer_solver, inverse_scaler=inverse_scaler, stack_samples=stack_samples, denoise=True)
